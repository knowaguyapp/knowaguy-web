...
export default function HomePage() {
  ...
  const isAuthDisabled =
    authLoading || email.trim().length === 0 || password.trim().length === 0;

const filteredProfiles = profiles.filter((p) => {
  // Never show a profile with no id
  if (!p.id) return false;

  // Optional: hide yourself from the list
  if (userId && p.id === userId) return false;

  const q = searchQuery.trim().toLowerCase();
  const cityFilter = filterCity.trim().toLowerCase();
  const modeFilter = filterMode.trim().toLowerCase();

  const name = (p.displayName || "").toLowerCase();
  const headlineText = (p.headline || "").toLowerCase();
  const cityText = (p.city || "").toLowerCase();
  const modeText = (p.mode || "").toLowerCase();
  const lookingForText = (p.lookingFor || "").toLowerCase();
  const skillsText = (p.skills || []).join(" ").toLowerCase();
  const plugsText = (p.plugs || []).join(" ").toLowerCase();

  // Search: if searchQuery exists, it must match any of these fields
  if (q) {
    const matchesSearch =
      name.includes(q) ||
      headlineText.includes(q) ||
      cityText.includes(q) ||
      modeText.includes(q) ||
      lookingForText.includes(q) ||
      skillsText.includes(q) ||
      plugsText.includes(q);

    if (!matchesSearch) return false;
  }

  // City filter: simple contains match
  if (cityFilter && !cityText.includes(cityFilter)) return false;

  // Mode filter: simple contains match
  if (modeFilter && !modeText.includes(modeFilter)) return false;
...
async function handlePasswordReset() {
  ...
  if (!trimmedEmail) {
    setAuthError("Please enter your email first to reset your password.");
    return;
  }
  ...
  try {
    ...
    setResetMessage(
      "If there is an account with that email we sent a reset link."
    );
  } catch (err: any) {
    console.error(err);
    // For security reasons use the same message even if email is not found
    setResetMessage(
      "If there is an account with that email we sent a reset link."
    );
  }
...
async function handleCopyContact(profile: UserProfile) {
  if (!profile.contact) return;
  try {
    await navigator.clipboard.writeText(profile.contact);
    setCopiedUserId(profile.id);
    setTimeout(() => {
      setCopiedUserId((current) => (current === profile.id ? null : current));
    }, 1500);
  } catch (err) {
        ...
        <div className="grid flex-1 gap-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] md:items-start">
                ...
                <div className="mb-4 space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="space-y-1.5">
                      ...
                      <input
                        className={`w-full rounded-lg border bg-slate-950 px-3 py-2 text-sm outline-none focus:border-slate-400 ${
                    ...
                    {authMode === "login" && (
                      <button
                        ...
                        disabled={authLoading || email.trim().length === 0}
                        className="mt-1 text-[11px] text-slate-300 underline disabled:opacity-50"
                      >
                        Forgot password
                      </button>
                    )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="e.g. @yourhandle on IG, or email, or preferred contact link"
                      />
                    </div>
          ...
          <section className="flex max-h-[640px] flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-5 md:p-6">
            ...
            <div className="mb-3 space-y-2">
              <input
                className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs outline-none focus:border-slate-400"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search name, city, mode, skills or plugs"
              />
              <div className="flex flex-col gap-2 text-[11px] text-slate-300 md:flex-row">
                <div className="flex-1 space-y-1">
                  <label className="block text-[10px] uppercase tracking-wide text-slate-400">
                    City filter
                  </label>
                  <input
                    className="w-full rounded-lg border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs outline-none focus:border-slate-400"
                    value={filterCity}
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. nightlife, founder, creator"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-1 text-sm">
              ...
              {profiles.length > 0 && filteredProfiles.length === 0 && (
                <p className="text-xs text-slate-500">
                  No profiles match your filters. Try clearing search or filters.
                </p>
              )}
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5 text-[10px]">
                        ...
                        {p.contact && (
                          <div className="mt-1 flex items-center justify-between gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => handleCopyContact(p)}
                              className="text-[10px] text-slate-200 underline"
                            >
                              {copiedUserId === p.id ? "Copied" : "Copy"}
                            </button>
                          </div>
                        )}
                          </div>
            ...
            <p className="mt-4 text-[11px] text-slate-500">
              ...
              <div>
                ...
                <p>
                  A simple card for what you do and what you have access to. Nightlife and business in the same network.
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <a
                  href="mailto:knowaguyhq@gmail.com"
